...
void drawAdvert() {
	...
	#define ADVERT_Y 50
	char str[] = "eth v" ETH_VERSION " - *nixCoders Team";
	//create window
	createWindow(&advertwin, "advertwin");
// -- can't be defined as static even if stored in cfg	
	Vector4Copy(colorDkGrey, advertwin.bgColor);
...
void drawIrcFireteam() {
	...
	#define IRCFT_BACKCOLOR (vec4_t){ 0.0f, 0.0f, 0.0f, 0.4f }
	#define IRCFT_BORDERCOLOR colorBlack
	#define IRCFT_TEXT "eth-tracker"
	...
	for (; count < (sizeof(ircBuddies) / sizeof(ircBuddy_t)); count++) {
		if (ircBuddies[count].infoValid) {
			buddiesNumbers++;
			int w = sizeTextWidth(IRCFT_FONT_SIZE, ircBuddies[count].nick, &eth.fontFixed);
	...
	if (sizeTextWidth(IRCFT_FONT_SIZE, IRCFT_TEXT, &eth.fontVariable) > max)
		width += sizeTextWidth(IRCFT_FONT_SIZE, IRCFT_TEXT, &eth.fontFixed);
	else
		width += max;
	...
	drawText(IRCFT_SPACE + ircFTwin.posX, y, IRCFT_FONT_SIZE, IRCFT_TEXT, colorRed, qfalse, qtrue, &eth.fontFixed);
	y += lineHeight;
		...
		if (buddy->infoValid) {
			...
			if (isOnServer) {
				...
				if (eth.clientInfo[buddy->playerID].team == TEAM_ALLIES)
					...
					flag = eth.bg_axispin;
				if (flag)
					drawPic(ircFTwin.posX + width - IRCFT_SPACE - sizeTextHeight(IRCFT_FONT_SIZE, &eth.fontFixed) - 2, y - 2, sizeTextHeight(IRCFT_FONT_SIZE, &eth.fontFixed) + 4, sizeTextHeight(IRCFT_FONT_SIZE, &eth.fontFixed) + 4, flag);
				if (nicks)
					drawText(ircFTwin.posX + IRCFT_SPACE + max, y, IRCFT_FONT_SIZE, buddy->ftString, colorWhite, qfalse, qtrue, &eth.fontFixed);
			}
			drawText(IRCFT_SPACE + ircFTwin.posX, y, IRCFT_FONT_SIZE, buddy->nick + sizeof(IRC_BOT_PREFIX) - 1, isOnServer ? colorWhite : colorDkGrey, qfalse, qtrue, &eth.fontFixed);
			y += lineHeight;
		}
	}
...
void drawIrcChat() {
	...
	#define IRC_CHAT_TEXT_COLOR colorYellow
	...
	if (ircMsgsGeneral.show) {
		...
		int lineHeight = sizeTextHeight(IRC_CHAT_TEXT_SIZE, &eth.fontFixed) + IRC_CHAT_SPACE;
		...
		while (ircmsgcount >= 0) {
			...
			if (ircmsgtext)	
				drawText(ircchatwin.posX + IRC_CHAT_SPACE, y, IRC_CHAT_TEXT_SIZE, ircmsgtext, IRC_CHAT_TEXT_COLOR, qfalse, qtrue, &eth.fontFixed);
			y += lineHeight;
			ircmsgcount--;
		}
	}
...
void drawIrcTopic() {
	...
	irctopicwin.windowH = sizeTextHeight(IRCTOPIC_SIZE,&eth.fontFixed);
	irctopicwin.windowW = sizeTextWidth(IRCTOPIC_SIZE, str, &eth.fontFixed);
...
// 		Riffle nade esp
		...
		// Ammo esp
// 		} else if (IS_AMMO(entityNum)) {
// 			drawText(entity->screenX - 4, entity->screenY, 0.10f, "A", colorGreen, qfalse, qtrue, &eth.fontFixed);
		//Medic Pack esp
// 		} else if (IS_HEALTH(entityNum)) {
// 			drawText(entity->screenX - 4, entity->screenY, 0.10f, "H", colorGreen, qfalse, qtrue, &eth.fontFixed);
		// Mortar esp
