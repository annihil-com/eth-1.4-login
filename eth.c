...
void autoRequestMedic() {
	...
	if (eth.cg_entities[eth.cg_clientNum].currentState->eFlags & EF_DEAD) {
		...
		if (!vsay)
			...
			syscall_CG_SendConsoleCommand("vsay FTReviveMe\n");
	...
	// If need health
		...
		if (!vsay)
			...
			syscall_CG_SendConsoleCommand("vsay FTHealMe\n");
...
// Calc muzzle
// kobj fixed the muzzle thing :D (Hooked server bullet_fire, and they match)
void setCurrentMuzzle() {
	VectorCopy(eth.cg_refdef.vieworg, eth.muzzle);
	if(eth.cg_snap->ps.viewangles[PITCH] > 30 && (eth.cg_snap->ps.eFlags & EF_CROUCHING))
		eth.muzzle[2] += 30.0 - eth.cg_snap->ps.viewheight;
			...
			switch (in[a+1]) {
				...
				case 'c':
					...
					case 'R': {
						char *msg;
						if (eth.killCountNoDeath == 10) {
								msg = "^3is on a  ^2KILLING SPREE!!! ^3(10 frags in a row)";
						}
						else if  (eth.killCountNoDeath == 15) {
								msg = "^3is ^2DOMINATING!!! ^3(15 frags in a row)";
						}
						else if  (eth.killCountNoDeath == 20) {
								msg = "^3is on a ^2RAMPAGE!!! ^3(20 frags in a row)";
						}
						else if  (eth.killCountNoDeath == 25) {
								msg = "^3is ^2UNSTOPPABLE!!! ^3(25 frags in a row)";
						}
						else if  (eth.killCountNoDeath == 30) {
								msg = "^3is ^2WICKED SICK!!! ^3(30 frags in a row)";
						}
						else if  (eth.killCountNoDeath == 35) {
								msg = "^3is on a MEGA KILLINGSPREE!!! ^3(35 frags in a row)";
						}
						else if  (eth.killCountNoDeath == 40) {
								msg = "^3is OMFGHAXOR!!! ^3(40 frags in a row)";
						}
						else if  (eth.killCountNoDeath == 45) {
								msg = "^3I'M HARD AS FUCKING STEEL!!! ^3(45 frags in a row)";
						}
						else if  (eth.killCountNoDeath == 50) {
								msg = "^3NOBODY CAN STOP ME NOW!!! ^3(50 frags in a row)";
						}
						else
							break;
						if (eth.killCountNoDeath >= 2)
					...
					case 'C': {
						...
						if (!randmsg)
							killmsg = strncpy(ksFormat, " ^2...Suprise,  Suprise, asshole^1!",sizeof(ksFormat));
			
						else if (randmsg == 1)
							killmsg = strncpy(ksFormat, " ^7You're a joke^1!",sizeof(ksFormat));
						else if (randmsg == 2)
							killmsg = strncpy(ksFormat, "^2... suck it down^1!",sizeof(ksFormat));
						else if (randmsg == 3)
							killmsg = strncpy(ksFormat, "^7, go home!",sizeof(ksFormat));
						else if (randmsg == 4)
							killmsg = strncpy(ksFormat, "^2...Suprise,  Suprise, asshole^1!!",sizeof(ksFormat));
						else if (randmsg == 5)
							killmsg = strncpy(ksFormat, "^7, shit happens.",sizeof(ksFormat));
						else if (randmsg == 6)
							killmsg = strncpy(ksFormat, "^7 is now in a better place!",sizeof(ksFormat));
						else if (randmsg == 7)
							killmsg = strncpy(ksFormat, "^2 is no more^1.",sizeof(ksFormat));
						else if (randmsg == 8)
							killmsg = strncpy(ksFormat, "^7, Come get some^1.",sizeof(ksFormat));
						else if (randmsg == 9)
							killmsg = strncpy(ksFormat, "^7 got in my way.",sizeof(ksFormat));
						else if (randmsg == 10)
							killmsg = strncpy(ksFormat, "^2... Eat shit and ^1Die!",sizeof(ksFormat));
						else if (randmsg == 11)
							killmsg = strncpy(ksFormat, "^7, you suck.",sizeof(ksFormat));
						else if (randmsg == 12)
