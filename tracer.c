...
/* to debug headbox using EV_RAILTRAIL (must have same defs as tracer funcs)

	this shows exactly where the hax thinks the hitbox is... therefor, changes
	to trace_head must also be made here
*/
/*
void makeRailTrail(vec3_t start, vec3_t end, vec3_t color)
{
	localEntity_t	*le;
	refEntity_t		*re;
	...
	le->startTime = eth.cg_time;
	le->endTime = eth.cg_time + 400;
	le->lifeRate = 1.0 / ( (float)le->endTime - (float)le->startTime );

	re->shaderTime = (float) eth.cg_time / 1000.0f;
	re->reType = RT_RAIL_CORE;
	re->customShader = eth.cgs_media_railCoreShader;
