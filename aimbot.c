...
// 				if (seth.value[VAR_HITBOX_DBG] == 3 && (eth.cg_time-eth.lastRailtrailTime > RAILTRAIL_INTERVAL)){
// 					debug_head_box( target );
// 					eth.lastRailtrailTime = eth.cg_time;
// 				}
			
				// Target found
				// kobj added hitbox aiming mode aimpoints
		...
		switch (eth.cg_snap->ps.weapon) {
			...
			case WP_FG42SCOPE:
				if (seth.value[VAR_AIMVECZ_TYPE] == VECZ_HITBOX) {
					...
					entity->head[0]=entity->head[0]+(rmin[0]+rmax[0])*0.7f;
// 					entity->head[1]=entity->head[1]+(rmin[1]+rmax[1])*0.7f;
// 					entity->head[2]=entity->head[2]+(rmin[2]+rmax[2])*0.7f;
