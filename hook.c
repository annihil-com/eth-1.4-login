...
orig_MSG_WriteDeltaUsercmdKey = detourFunction((void *)0x080C453C, eth_MSG_WriteDeltaUsercmdKey);
	...
	// Set banner/stats format
			if (!getenv("ETH_BANNER"))
				strncpy(bnFormat,"^neth-^n1.4  ^3nixCoders  ^2|  ^7Name: ^2[p]^*  ^2|  ^7Mode: ^2[M] ^2|  ^7Kills: ^2[t] ^2|  ^7Spree: ^2[c]  ^2|  ^7XP: ^2[x]",sizeof(bnFormat));
			else
				strncpy(bnFormat,getenv("ETH_BANNER"),sizeof(bnFormat));

	// Set deathspam
			if (!getenv("ETH_DEATHSPAM"))
				strncpy(dieFormat," ^2[K] ^7Killed me.",sizeof(dieFormat));
			else
...
//        			history[[player][0].air_state = es->groundEntityNum;

//			    printf("%f %f %f - %d || %f %f %f - %d \n",eth.cg_snap->entities[i].pos.trBase[0],eth.cg_snap->entities[i].pos.trBase[1],eth.cg_snap->entities[i].pos.trBase[2],eth.cg_snap->entities[i].pos.trType,eth.cg_snap->entities[i].apos.trDelta[0], eth.cg_snap->entities[i].apos.trDelta[1], eth.cg_snap->entities[i].apos.trDelta[2], eth.cg_snap->entities[i].apos.trType);
			    }
			}
...
//		pcor = eth.cg_time - eth.cg_snap->serverTime;
//		if (pcor > 0) pcor *= -1;
